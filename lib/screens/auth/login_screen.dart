...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _login() async {
    ...
    try {
      final userCredential = await _auth.signIn(_emailController.text.trim(), _passwordController.text.trim());
      
      // Lấy role của user từ Firestore
      final role = await _auth.getUserRole(userCredential.user!.uid);
      
      if (mounted) {
        // Nếu user là admin, chuyển sang AdminDashboardScreen
        if (role == 'admin') {
          ...
          // Nếu là customer hoặc không có role, vào MainScreen
      ...
      if (mounted) {
        showAppSnackBar(context, SnackBar(content: Text("Lỗi đăng nhập: ${e.toString()}")));
      }
    } finally {
