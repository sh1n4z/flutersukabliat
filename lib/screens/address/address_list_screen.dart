...
import 'edit_address_screen.dart'; // Đã đổi sang file xịn, xóa sổ add_address
...
class AddressListScreen extends StatelessWidget {
              ...
              builder: (context, snapshot) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _buildAddressCard(context, address, user?.uid);
                  },
                );
              },
            ),
  ...
  Widget _buildAddressCard(BuildContext context, AddressModel address, String? userId) {
    return Container(
          ...
          children: [
            Row(
              ...
              children: [
                ...
                InkWell(
                  onTap: () {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => EditAddressScreen(address: address)));
                  },
                  child: Padding(
                    padding: const EdgeInsets.all(4.0),
                    child: Icon(Icons.edit_outlined, size: 20, color: AppColors.woodAccent.withOpacity(0.8)),
                  ),
                ),
                // Nút Xóa
                InkWell(
                  onTap: () async {
                    if (userId != null) {
                      await FirebaseFirestore.instance.collection('users').doc(userId).collection('addresses').doc(address.id).delete();
                      if (context.mounted) {
                        showAppSnackBar(context, const SnackBar(content: Text('Đã xóa địa chỉ')));
                      }
                    }
                  },
                  child: Padding(
      ...
      child: SafeArea(
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: () {
              // Gọi sang form thêm mới (không truyền address)
